    ...
    public partial class frmPosDayLog : Form
    {
        ...
        private async void btnVoid_Click(object sender, EventArgs e)
        {
            ...
            var confirm = MessageBox.Show(
                $"Void this {row.Total:N2} sale by {row.Cashier}?\n\nIts stock will be returned to the branch. This cannot be undone.",
            ...
            try
            {
                await BranchApiClient.VoidSaleAsync(branchName, row.ClientSaleId, voidedBy.Trim());
            }
            catch (Exception ex)
            {
            ...
            // best-effort - a local write failure here is only cosmetic (the office report and Sales
            // History read the server's authoritative voided flag), so it must NOT be reported as a
            // void failure the way a network error is.
