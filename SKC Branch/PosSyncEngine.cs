...
namespace SKC_Branch
{
    ...
    // Push-then-pull sync for the POS. Push first so the pulled stock snapshot already
        ...
        public static async Task<SyncOutcome> SyncAsync(string branch)
        {
            ...
            try
            {
                ...
                if (pending.Count > 0)
                {
                    ...
                    try
                    {
                        results = await BranchApiClient.PushSalesAsync(pending);
                    }
                    catch
                    {
                ...
                try
                {
                    var stock = await BranchApiClient.GetMyStockAsync(branch);
