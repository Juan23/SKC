...
{
    // The branch's own sales history over a date range, reading the same /api/sales endpoints the
    // office Branch Sales Report uses (no new server endpoint). Voided sales are greyed and excluded
    // from the total; shortfall sales are flagged. Read-only, requires connectivity to load.
    public partial class frmBranchSalesHistory : Form
    {
        ...
        private void frmBranchSalesHistory_Load(object sender, EventArgs e)
        {
        ...
        private async void LoadSales()
        {
            ...
            try
            {
                currentSales = await BranchApiClient.GetBranchSalesAsync(branchName, start, end);
